...
1. **Gravity + jump** - applies gravity each tick when not grounded. Jump buffer: pressing jump sets `_jump_buffer = JUMP_BUFFER_TIME`; each tick decrements by delta; fires jump when grounded and `_jump_buffer > 0`, then resets buffer to 0.
...
- `characters/player/player.gd` - new file (this session)
